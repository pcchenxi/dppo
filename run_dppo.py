...
class PPO(object):
    ...
    def _build_feature_net(self, name, input_state, trainable):
        ...
        with tf.variable_scope(name):
            ...
            h = tf.layers.conv2d(inputs=reshaped_grid, filters=16, kernel_size=[8, 8], strides = 4, name = 'conv1', padding="valid", kernel_initializer=ortho_init(np.sqrt(2)), activation=tf.nn.relu, trainable=trainable )
            h1 = tf.layers.conv2d(inputs=h, filters=32, kernel_size=[4, 4], strides = 2, name = 'conv2', padding="valid", kernel_initializer=ortho_init(np.sqrt(2)), activation=tf.nn.relu, trainable=trainable )
            h2 = tf.layers.conv2d(inputs=h1, filters=32, kernel_size=[3, 3], strides = 1, name = 'conv3', padding="valid", kernel_initializer=ortho_init(np.sqrt(2)), activation=tf.nn.relu, trainable=trainable )
    ...
    def _build_anet(self, name, input_state, trainable):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('actor_critic'):
                h4 = tf.layers.dense(feature, 512, tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)), name = 'fc1', trainable=trainable)

                if isinstance(Action_Space, gym.spaces.Box):
                    print('continue action',A_DIM, S_DIM)
                    pi = tf.layers.dense(h4, A_DIM, kernel_initializer=ortho_init(0.01), name = 'fc_pi', trainable=trainable)
